...
% figure is drawn
tmpfilename = 'Dormancy_Dyn_host_demography_Dfrac_&_Sfrac';
...
pvec=linspace(0,1/48,3) %1/24 approximately 0.042
...
f(2)=phi*S1*V1*delta-p*D1-mu*I1;
...
tmph=plot(T,Dvec(1,:)./Nvec(1,:),'r-',T,Dvec(2,:)./Nvec(2,:),'r--',T,Dvec(3,:)./Nvec(3,:),'r:',T,Svec(1,:)./Nvec(1,:),'b-',T,Svec(2,:)./Nvec(2,:),'b--',T,Svec(3,:)./Nvec(3,:),'b:')
...
%tmph=plot(T,Nvec(1,:),'r-',T,Nvec(2,:),'r--',T,Nvec(3,:),'r:')
%,T,Nvec(4,:),'r-.',T,Nvec(5,:),'r--.')
...
ylabel('Dormant cell density fraction, $D(t)/N(t)$','fontsize',20,'verticalalignment','bottom','interpreter','latex');
