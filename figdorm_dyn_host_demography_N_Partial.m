...
% figure is drawn
tmpfilename = 'Dormancy_Dyn_host_demography_N_Partial';
...
pvec=linspace(0,1/24,5) %1/24 approximately 0.042
Svec=zeros(length(pvec),length(0:0.1:48));
Dvec=zeros(length(pvec),length(0:0.1:48));
Ivec=zeros(length(pvec),length(0:0.1:48));
