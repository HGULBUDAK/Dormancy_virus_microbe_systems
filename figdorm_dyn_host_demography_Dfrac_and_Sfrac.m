...
% figure is drawn
tmpfilename = 'Dormancy_Dyn_host_demography_Dfrac_and_Sfrac';
...
tmph=plot(T,Dvec(1,:)./Nvec(1,:),'r-',T,Dvec(2,:)./Nvec(2,:),'r--',T,Dvec(3,:)./Nvec(3,:),'r:',T,Dvec(4,:)./Nvec(4,:),'r-.',T,Svec(1,:)./Nvec(1,:),'b-',T,Svec(2,:)./Nvec(2,:),'b--',T,Svec(3,:)./Nvec(3,:),'b:',T,Svec(4,:)./Nvec(4,:),'b-.')
legend({'\gamma=0','\gamma\approx 0.014','\gamma \approx 0.028  ','\gamma \approx 0.042'},'Position',[0.64,0.65,0.25,0.1],'FontSize',15,'FontWeight','bold')
...
ylabel({'Dormant/susceptible cell density fraction', '\qquad\qquad\quad $D(t)/N(t), S(t)/N(t)$'},'fontsize',20,'verticalalignment','bottom','interpreter','latex');
